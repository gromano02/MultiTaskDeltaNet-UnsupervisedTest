...
def CDEva():
    ...
    #PATH = '/home/yun13001/code/Carbon/tianyu_new_data/unet_github/checkpoints/test_cv3_all_aug/best_a1.pt'
    PATH = '/home/yun13001/code/Carbon/tianyu_new_data/unet_github/checkpoints/test_cv3_all_aug/best_a2.pt'
    ...
    #for batch_id, batch in enumerate(dataloaders['test'], 0):
        with torch.no_grad():
            ...
            pred1 = torch.argmax(G_pred1, dim=1, keepdim=True)
            pred2 = torch.argmax(G_pred2, dim=1, keepdim=True)
            pred1 = utils.make_numpy_grid_lb(pred1)
            pred2 = utils.make_numpy_grid_lb(pred2)
            ...
            vis_gt1 = palette[gt1.ravel()].reshape(vis_input.shape)
            vis_gt2 = palette[gt2.ravel()].reshape(vis_input.shape)

            #vis_dir = '/home/yun13001/code/Carbon/tianyu_new_data/unet_github/vis/test_cv3_all_aug/a1_save'
            vis_dir = '/home/yun13001/code/Carbon/tianyu_new_data/unet_github/vis/test_cv3_all_aug/a2_save'

            vis = np.concatenate([vis_input, vis_input2, vis_gt1, vis_pred1, vis_gt2, vis_pred2], axis=0)
            ...
            sns.heatmap(cm1, annot=True, ax = ax, fmt = 'g', cmap=colormap, annot_kws={'size': 40})
            ...
            sns.heatmap(cm2, annot=True, ax = ax, fmt = 'g', cmap=colormap, annot_kws={'size': 40})
            # labels, title and ticks
            ax.set_xlabel('Predicted', fontsize=35)
            ax.xaxis.set_label_position('bottom')
            #plt.xticks(rotation=90)
        ...
        #current_score1 = running_metric1.update_cm(pr=G_pred1.cpu().numpy(), gt=target1.cpu().numpy())  ### only f2 score
        current_score1 = running_metric1.undate_score(pr=G_pred1.cpu().numpy(), gt=target1.cpu().numpy())   ### all metrics 
        ...
        #current_score2 = running_metric2.update_cm(pr=G_pred2.cpu().numpy(), gt=target2.cpu().numpy())  ### only f2 score
        current_score2 = running_metric2.undate_score(pr=G_pred2.cpu().numpy(), gt=target2.cpu().numpy()) ### all metrics 
