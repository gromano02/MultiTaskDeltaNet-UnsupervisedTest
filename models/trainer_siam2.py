...
def CDTrainer():
    ...
    #optimizer_G = optim.AdamW(net_G.parameters(), lr=0.00037037312300132946, betas=(0.9, 0.999), weight_decay=0.01)  ###cv_all_pair(workstation3)
    #optimizer_G = optim.AdamW(net_G.parameters(), lr=0.000646846783774518, betas=(0.9, 0.999), weight_decay=0.01)
    ...
    for epoch_id in range(epoch_to_start, max_num_epochs):
        ...
        for batch_id, batch in enumerate(dataloaders['train'], 0):
            ...
            if epoch_id>=2000: #0(reg)  #1000(no reg)   ##### Epoch to apply regularization 
               ...
               lam = 0.6300668403918332
               #lam = 4.34819e-05
               #lam = 4.348187498605464e-05
