...
from models.trainer_siam_raytune2 import *
...
def main(num_samples=10, max_num_epochs=200, gpus_per_trial=1):
    ...
    #    "lam": tune.loguniform(1e-5, 10000),
    #}
    #config = {
    #    "lr": tune.loguniform(1e-5, 0.004),
    #    "batch_size": tune.choice([ 4, 8, 16]),
    #    "lam": tune.loguniform(1e-5, 500),
    #}
    #config = {
    #    "lr": tune.loguniform(0.0002, 0.004),
    #    "batch_size": tune.choice([8, 16]),
    #    "lam": tune.loguniform(1.5e-5, 130),
    #}

    #cv:
    #config = {
    #    "lr": tune.loguniform(0.0001, 0.008),
    #    "batch_size": tune.choice([8, 16]),
    #}
    #cv2:
    #config = {
    #    "lr": tune.loguniform(0.0006, 0.008),
    #    "batch_size": tune.choice([16]),
    #}

    #unet1:
    config = {
        "lr": tune.loguniform(0.0001, 0.003),
        "batch_size": tune.choice([8, 16]),
    }
    ...
    scheduler = ASHAScheduler(
        ...
        grace_period=15,       #10,15,20     ####train each trial at least for n epochs
        reduction_factor=2,)   #1.6  ####if =2, only 50% of all trials are kept each time they are reduced
...
if __name__ == "__main__":
    ...
    #main(num_samples=100, max_num_epochs=400, gpus_per_trial=1)    ### 200
    main(num_samples=50, max_num_epochs=500, gpus_per_trial=2)    ### 100,500
